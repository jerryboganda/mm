...
export const SOURCE_DOCX_SHA256 = "f94027611ab71565c9dfd689046bb4a24db921b97ef1453416d5acfa140ed605";
...
export function isImportedBookBlock(block: { contentType?: string | null } | null | undefined): boolean {
...
export function parseReleaseMarker(html: string): ReleaseMarkerData | null {
  if (!html) return null;
  const match = html.match(/class=["']mm-release-marker["'][^>]*data-mm-release=["']([^"']+)["'][^>]*data-mm-topic=["']([^"']+)["']/i)
    || html.match(/data-mm-release=["']([^"']+)["'][^>]*data-mm-topic=["']([^"']+)["']/i);
