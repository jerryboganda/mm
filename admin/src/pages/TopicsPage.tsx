...
export default function TopicsPage() {
  ...
  const load = async () => {
    try {
      if (bookId) {
        const [ts, books] = await Promise.all([
          api.get<Topic[]>(`/admin/content/books/${bookId}/topics`),
          api.get<Book[]>("/admin/content/books"),
        ]);
        setTopics(ts);
        const b = books.find((x: any) => x.id === bookId);
        if (b) setParentBook(b);
      } else if (chapterId) {
  ...
  const handleSave = async () => {
    ...
    try {
      if (editTopic) {
        ...
        await api.post("/admin/content/topics", {
          ...form,
          ...(chapterId ? { chapterId } : {}),
          ...(bookId ? { bookId } : {}),
        });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <Link
          to={bookId ? "/books" : chapter ? `/books/${chapter.bookId}/chapters` : "/books"}
        ...
        <div className="flex-1">
          <p className="text-sm text-gray-500">
            {parentBook ? `Book: ${parentBook.title}` : chapter ? `Chapter: ${chapter.title}` : "Topics"}
          </p>
