...
export default function ChaptersPage() {
  ...
  const load = async () => {
    try {
      const [b, chs, subs] = await Promise.all([
        api
          .get<Book[]>(`/admin/content/books`)
          .then((books: any) => books.find((b: Book) => b.id === bookId)),
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editChapter) {
        await api.put(`/admin/content/chapters/${editChapter.id}`, payload);
      } else {
        await api.post("/admin/content/chapters", { ...payload, bookId });
      }
      setShowForm(false);
      load();
    } catch (err: any) {
  ...
  const handleDelete = async (ch: Chapter) => {
    if (
      ...
      return;
    try {
      await api.delete(`/admin/content/chapters/${ch.id}`);
      load();
    } catch (err: any) {
      alert(err.message);
    }
  ...
  const togglePublish = async (ch: Chapter) => {
    try {
      await api.put(`/admin/content/chapters/${ch.id}`, {
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className="text-xs text-gray-400 mt-1">
                  Manage subjects under Books › Subjects.
                </p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      className={`px-2 py-0.5 rounded-full text-xs font-medium ${ch.isPublished ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500"}`}
                    >
                      {ch.isPublished ? "Published" : "Draft"}
                    </span>
                    {ch.subjectId && (
                      <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-indigo-50 text-indigo-700">
                        {subjects.find((s) => s.id === ch.subjectId)?.title ??
                          "Subject"}
                      </span>
                    )}
                  </div>
