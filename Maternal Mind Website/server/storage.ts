import {
  ...
  type InsertInstitutionalRequest,
} from "@shared/schema";
...
export interface IStorage {
  ...
  createAccountDeletionRequest(
    request: InsertAccountDeletionRequest,
  ): Promise<AccountDeletionRequest>;
  createInstitutionalRequest(
    request: InsertInstitutionalRequest,
...
export class MemStorage implements IStorage {
  ...
  constructor() {
    ...
    this.accountDeletionRequests = new Map();
  ...
  async createNewsletterEntry(
    entry: InsertNewsletter,
  ): Promise<NewsletterEntry> {
    const id = randomUUID();
    const newsletterEntry: NewsletterEntry = {
      ...entry,
      id,
      createdAt: new Date(),
    };
  ...
  async createAccountDeletionRequest(
    ...
    this.accountDeletionRequests.set(id, accountDeletionRequest);
    return accountDeletionRequest;
  }

  async createInstitutionalRequest(
    request: InsertInstitutionalRequest,
  ): Promise<InstitutionalRequest> {
    const id = randomUUID();
