...
const filePath = path.resolve(process.cwd(), 'server/services/subscription-service.ts');
...
content = content.replace(/const\s+\[([a-zA-Z0-9_]+)\]\s*=\s*await\s+db\s*\n?\s*\.insert\(/g, 'await db.insert(');
content = content.replace(/const\s+\[([a-zA-Z0-9_]+)\]\s*=\s*await\s+db\s*\n?\s*\.update\(/g, 'await db.update(');
