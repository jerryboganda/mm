...
async function verify() {
  ...
  const [docBlocks] = await conn.query("SELECT count(*) as c FROM content_blocks WHERE type = 'document_html'");
  const [users] = await conn.query("SELECT count(*) as c FROM users");
  
  // Sample check topic 1 content
  const [sampleBlock] = await conn.query("SELECT content FROM content_blocks WHERE topic_id = 't-mm-01-001' LIMIT 1");
  const hasReleaseMarker = sampleBlock.length > 0 && sampleBlock[0].content.includes('data-mm-release="f94027611ab71565c9dfd689046bb4a24db921b97ef1453416d5acfa140ed605"');
  ...
  console.log(`Sample Topic t-mm-01-001 Release Marker Verified: ${hasReleaseMarker}`);
  console.log("=========================================");
