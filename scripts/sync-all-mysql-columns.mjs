...
async function sync() {
  ...
  try {
    ...
    if (!appSettingColNames.includes("id")) {
      ...
      await conn.query(
        "ALTER TABLE app_settings ADD COLUMN id VARCHAR(191) NOT NULL DEFAULT (UUID()) FIRST",
      );
    }
    ...
    if (!progressColNames.includes("completed_at")) {
      ...
      await conn.query(
        "ALTER TABLE user_progress ADD COLUMN completed_at DATETIME",
      );
    }
    ...
    if (!topicColNames.includes("is_paid")) {
      ...
      await conn.query(
        "ALTER TABLE topics ADD COLUMN is_paid BOOLEAN NOT NULL DEFAULT false",
      );
    }
    if (!topicColNames.includes("last_reviewed_at")) {
      await conn.query(
        "ALTER TABLE topics ADD COLUMN last_reviewed_at DATETIME",
      );
    }
    ...
    // 6. recent_activity
    try {
      const [activityCols] = await conn.query(
        "SHOW COLUMNS FROM recent_activity",
      );
      ...
      if (!activityColNames.includes("topic_id")) {
        ...
        await conn.query(
          "ALTER TABLE recent_activity ADD COLUMN topic_id VARCHAR(191)",
        );
      }
      if (!activityColNames.includes("viewed_at")) {
        console.log("[*] Adding viewed_at column to recent_activity...");
        await conn.query(
          "ALTER TABLE recent_activity ADD COLUMN viewed_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP",
        );
      }
    } catch {}
