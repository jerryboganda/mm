  ...
  while (i < lines.length) {
    ...
    if (/^\s*const\s+\[/.test(line) && /await\s+db\s*$/.test(trimmed)) {
      ...
      while (j < lines.length) {
        ...
        if (nextTrimmed.startsWith('.insert(') || nextTrimmed.startsWith('.update(')) {
        ...
        if (nextTrimmed.endsWith(';') && !nextTrimmed.startsWith('.') && !nextTrimmed.startsWith('//')) {
          break;
        }
        if (nextTrimmed.endsWith(';') && (nextTrimmed.startsWith('.where(') || nextTrimmed.startsWith('.values('))) {
