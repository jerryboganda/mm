...
# AUTOMATIC MAIN AGENT PASS-THROUGH & SKILL ROUTING

1. **Mandatory Execution for ALL Prompts**:
   - Every single user prompt — whether big or small, simple or complex — MUST automatically pass through and be processed strictly by the **Main Agent** under this Core Operating Protocol.
   - The user NEVER needs to ask, command, or remind the agent to use the Main Agent or any skill.
